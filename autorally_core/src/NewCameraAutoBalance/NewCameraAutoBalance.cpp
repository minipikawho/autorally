...
PLUGINLIB_EXPORT_CLASS(autorally_core::NewCameraAutoBalance, nodelet::Nodelet)

namespace autorally_core
{
NewCameraAutoBalance::NewCameraAutoBalance()
{
}

void NewCameraAutoBalance::onInit()
{
    ...
    cb = boost::bind(&NewCameraAutoBalance::configCallback, this, _1, _2);
    ...
    sub_ = nh.subscribe("left_camera/image_color/compressed", 100, &NewCameraAutoBalance::imageCallback, this);
...
void NewCameraAutoBalance::configCallback(const camera_auto_balance_paramsConfig &config, uint32_t level)
{
...
void NewCameraAutoBalance::cameraParametersInitialization() {
    prop_.onOff = true;
...
void NewCameraAutoBalance::imageCallback(const sensor_msgs::Image::ConstPtr& msg)
{
...
void NewCameraAutoBalance::autoExposureControl(const cv_bridge::CvImageConstPtr &cv_ptr) {
...
double NewCameraAutoBalance::saturate(double x, double min, double max){
    if (x < min)
        x = min;
    else if (x > max)
        x = max;
...
double NewCameraAutoBalance::MSV(cv_bridge::CvImageConstPtr cv_ptr){
...
void NewCameraAutoBalance::histogram(cv_bridge::CvImageConstPtr cv_ptr, std::vector<int> &hist, cv::Rect &roi,
...
void NewCameraAutoBalance::plotHistogram(std::vector<int> &hist) {
    int max = 0;
    std::vector<int>::iterator it;
